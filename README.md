...
`ESI-Tool` 是一套面向非计算机专业人员的 Windows 桌面统计工具，用来把 `6` 期 ESI 附表自动整理成年度统计工作簿、详细去重总表和配套报告，并支持生成可分发的 Windows 安装包。
...
- `2025年度ESI高水平论文统计结果.xlsx`
  适合直接查看 `表12-表14`
- `2025年度ESI高水平论文去重总表.xlsx`
  包含 `去重总表、统计汇总、拼音待核明细、逐期原始记录、兼容性检查`
...
当前版本按 `2025` 年度统计流程设计，优先识别以下附表命名模式：
...
如果文件名或 sheet 名与上述模式不完全一致，程序会先尝试自动匹配；匹配结果和提示会写入详细总表中的 `兼容性检查` sheet。
...
- 当前版本主要针对 `2025` 年口径
- 如果附表表头字段名称变化过大，或模板中作者分布表结构完全重排，仍可能需要人工调整规则
